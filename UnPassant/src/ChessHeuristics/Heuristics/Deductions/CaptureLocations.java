...
public class CaptureLocations extends AbstractDeduction {
    ...
    private int pieceReductions(BoardInterface board, boolean white) {
        int y = white ? HeuristicsUtil.FINAL_RANK_Y : HeuristicsUtil.FIRST_RANK_Y;
                ...
                .filter(entry -> {
                    ...
                    if (map.size() == 1) {
                        ...
                        return this.piecePaths
                                .get(new Coordinate(Math.abs(HeuristicsUtil.FINAL_RANK_Y - x), pieceCoord.getY()))
        ...
        int ofWhichBishop = (int) ofWhichCaged.stream().filter(coordinate -> {
                    ...
                    boolean bishop = x == HeuristicsUtil.K_BISHOP_X || x == HeuristicsUtil.Q_BISHOP_X;
                    if (bishop) {
        ...
        Path ofWhichRook = Path.of(ofWhichCaged.stream()
                .filter(coordinate -> coordinate.getX() == HeuristicsUtil.Q_ROOK_X || coordinate.getX() == HeuristicsUtil.K_ROOK_X) // Is a rook
                .toList());

        int ofWhichQueen = ofWhichCaged.size() - ofWhichBishop - ofWhichRook.size();
        if (ofWhichQueen > 0) {
            nonPawnCaptures.add(new Coordinate(HeuristicsUtil.QUEEN_X, y));
        }
        ...
        List<BiPredicate<Coordinate, Coordinate>> predicates =
                new LinkedList<>(this.piecePaths.entrySet().stream()
                        ...
                        .filter(entry -> entry.getKey().getX() == HeuristicsUtil.Q_BISHOP_X || entry.getKey().getX() == HeuristicsUtil.K_BISHOP_X) //Is a Bishop
        ...
        if (!predicates.isEmpty()) {
            ...
            if (pawnCaptures(paths, white) == 0) {
                if (!predicates.isEmpty()) {
                    ...
                    Path.of(new Coordinate(HeuristicsUtil.Q_BISHOP_X, y), new Coordinate(HeuristicsUtil.K_BISHOP_X, y)).forEach(c -> {
    ...
    private int findInaccessibleRooks(BoardInterface board, boolean white, Path rooks) {
        Map<Integer, Path> reachable = Map.of(HeuristicsUtil.Q_ROOK_X, new Path(), HeuristicsUtil.K_ROOK_X, new Path());
        ...
        if (!rooks.isEmpty()) {
            rooks.forEach(coordinate2 -> board.getBoardFacts().getCoordinates(white, "pawn")
                    .stream().filter(coordinate -> white ? (coordinate.getY() >= HeuristicsUtil.BLACK_PAWN_Y)  : (coordinate.getY() <= HeuristicsUtil.WHITE_PAWN_Y))
                    .forEach(coordinate -> {
    ...
    private List<Coordinate> pawnCaptureLocations(boolean white, BoardInterface board) {
        ...
        int pawnsLostByPlayer = (HeuristicsUtil.MAX_PAWNS - board.getBoardFacts()
    ...
    private Map<Coordinate, List<Path>> everySingularPawnPath(boolean white) {
        ...
        HashMap<Coordinate, List<Path>> returnMap = new HashMap<>(
                pathsInUse.entrySet().stream()
                        //On ranks 2 or 5
                        .filter(entry -> entry.getKey().getY() == (white ? HeuristicsUtil.WHITE_ESCAPE_Y : HeuristicsUtil.BLACK_ESCAPE_Y))
                        .filter(entry -> pathsInUse //There is a pawn behind it
                                .containsKey(new Coordinate(entry.getKey().getX(), white ? HeuristicsUtil.WHITE_PAWN_Y : HeuristicsUtil.BLACK_PAWN_Y)))
