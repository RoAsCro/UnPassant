...
public class CombinedPawnMap extends AbstractDeduction {
    ...
    private boolean exclude(BoardInterface board, boolean white) {
        ...
        singleOriginPawns.forEach((opposingPawn, opposingPaths) -> checkedPlayerPaths.entrySet()
                ...
                .filter(innerEntry -> {
                    ...
                    if (y2 == HeuristicsUtil.FINAL_RANK_Y || y2 == HeuristicsUtil.FIRST_RANK_Y) {
    ...
    public static class PawnMap extends AbstractDeduction{
        ...
        protected void rawMap(BoardInterface board) {
            int start = Math.abs((this.white ? HeuristicsUtil.FIRST_RANK_Y : HeuristicsUtil.FINAL_RANK_Y) - 1);
            int increment = this.white ? 1 : -1;
            BoardReader reader = board.getReader();
            for (int y = 0; y < HeuristicsUtil.FINAL_RANK_Y - 1; y++) {
                reader.to(new Coordinate(0, start + y * increment));
                int finalY = y;
                reader.nextWhile(Coordinates.RIGHT, coordinate -> coordinate.getX() <= HeuristicsUtil.FINAL_RANK_Y, piece -> {
                    if (piece.getType().equals("pawn") && piece.getColour().equals(this.white ? "white"  : "black")) {
                        ...
                        for (int j = 0 ; j < potentialPaths ; j++) {
                            ...
                            if (x > HeuristicsUtil.FINAL_RANK_Y) {
                                break;
                            }
                            if (x < HeuristicsUtil.FIRST_RANK_Y) {
                                continue;
                            }
