...
public class PathfinderUtil {
    ...
    public Predicate<Path> firstRankCollision = path -> {
        ...
        return !(
                (coordinate.getY() == HeuristicsUtil.FIRST_RANK_Y || coordinate.getY() == HeuristicsUtil.FINAL_RANK_Y)
                        && !HeuristicsUtil.STANDARD_STARTS.get(coordinate.getX()).equals("rook")
    ...
    public Predicate<Coordinate> secondRankCollision = coordinate -> {
        ...
        if ((y == HeuristicsUtil.WHITE_PAWN_Y || y == HeuristicsUtil.BLACK_PAWN_Y)) {
            Map<Coordinate, List<Path>> map = this.detector.getPawnData().getPawnPaths(y == HeuristicsUtil.WHITE_PAWN_Y);
    ...
    public Predicate<Path> thirdRankCollision = path -> {
        ...
        if (y == HeuristicsUtil.WHITE_ESCAPE_Y || y == HeuristicsUtil.BLACK_ESCAPE_Y) {
            boolean white = y == HeuristicsUtil.WHITE_ESCAPE_Y;
            Path toCheck = this.detector.getPawnData().getPawnPaths(white).entrySet()
                    .stream().filter(e -> e.getKey().getY() == (white ? HeuristicsUtil.WHITE_ESCAPE_Y : HeuristicsUtil.BLACK_ESCAPE_Y))
    ...
    public Path findPiecePath(BoardInterface board, String pieceName,
        ...
        return Pathfinder.findShortestPath(
                ...
                start,
                (b, c) -> c.equals(target) || (c.getY() >= HeuristicsUtil.WHITE_ESCAPE_Y && c.getY() <= HeuristicsUtil.BLACK_ESCAPE_Y),
                board,
