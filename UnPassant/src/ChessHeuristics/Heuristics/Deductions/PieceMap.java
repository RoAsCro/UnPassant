...
public class PieceMap extends AbstractDeduction{
    ...
    public void deduce(BoardInterface board) {
        Arrays.stream(new int[]{HeuristicsUtil.Q_BISHOP_X, HeuristicsUtil.K_BISHOP_X, HeuristicsUtil.KING_X, HeuristicsUtil.QUEEN_X, HeuristicsUtil.Q_ROOK_X, HeuristicsUtil.K_ROOK_X}).forEach(x -> {
    ...
    private void pathToEnd(BoardInterface board, Map<String, Path> certainPromotions,
        ...
        this.promotedPieceMap.forEach((key, value) -> {
            ...
            coordinatesToRemove.forEach(value::remove);
        });
    }
    ...
    private void getCertainPromotionsHelper(BoardInterface board, boolean white,
        ...
        Arrays.stream(new int[]{HeuristicsUtil.Q_ROOK_X, HeuristicsUtil.Q_BISHOP_X, HeuristicsUtil.QUEEN_X}).forEach(x -> {
            String pieceName = HeuristicsUtil.STANDARD_STARTS.get(x);
    ...
    private void configureCastling(BoardInterface board) {
        ...
        ROOK_COORDS.forEach((r) -> {
            if (board.canCastle(r.getY() == HeuristicsUtil.FIRST_RANK_Y, r.getX() == HeuristicsUtil.Q_ROOK_X)) {
    ...
    private void configureCastlingPartTwo(BoardInterface board) {
        ...
        ROOK_COORDS.forEach((r) -> {
            if (board.canCastle(r.getY() == HeuristicsUtil.FIRST_RANK_Y, r.getX() == HeuristicsUtil.Q_ROOK_X)) {
                ...
                this.pieceMap.get(new Coordinate(Math.abs(r.getX() - HeuristicsUtil.K_ROOK_X), r.getY())).keySet().remove(r);
            }
        });
    }
    ...
    private void kingMovementUpdate(BoardInterface board){
        ...
        ROOK_COORDS.forEach(r -> {
            if (!board.getBoardFacts().getCoordinates(r.getY() == HeuristicsUtil.FIRST_RANK_Y, rook)
                    .contains(r)) {
                pieceData.setRookMovement(r.getY() == HeuristicsUtil.FIRST_RANK_Y, r.getX() == HeuristicsUtil.Q_ROOK_X,
                        true);
            }
        });
    ...
    private void kingDisplaced(BoardInterface board) {
        ...
        // Check queen
        Coordinate currentQueen = new Coordinate(HeuristicsUtil.QUEEN_X, HeuristicsUtil.FIRST_RANK_Y);
        boolean white = true;
        ...
        for (int i = 0 ; i < 2 ; i++) {
            if ((white && !this.detector.getPieceData().getKingMovement(true))
                ...
                if (this.pieceMap.containsKey(currentQueen) && ((this.pieceMap.get(currentQueen)
                        .keySet().stream().anyMatch(c -> disturbsKing(board, "queen", HeuristicsUtil.PIECE_CODES.get("queen"), finalCurrentQueen, c, finalWhite)))
                        || (this.pieceMap.get(currentQueen).isEmpty() && currentAllPieces && disturbsKing(board, "queen", HeuristicsUtil.PIECE_CODES.get("queen"), finalCurrentQueen, Coordinates.NULL_COORDINATE, white)))
                ) {
                    this.detector.getPieceData().setKingMovement(white, true);
                }
            }
            currentQueen = new Coordinate(HeuristicsUtil.QUEEN_X, HeuristicsUtil.FINAL_RANK_Y);
            white = false;
    ...
    private void pathFromOtherDirection(BoardInterface board, Map<Coordinate, Map<Coordinate, Path>> paths) {
        paths.forEach((key, value) -> {
            Path coordinatesToRemove = new Path();
            String pieceName = HeuristicsUtil.STANDARD_STARTS.get(key.getX());
            String pieceCode = key.getY() == HeuristicsUtil.FIRST_RANK_Y
                    ? pieceName.substring(0, 1).toUpperCase()
                    : pieceName.substring(0, 1).toLowerCase();
            value.keySet().forEach(origin -> {
                if (this.pathFinderUtil.findPiecePath(board, pieceName, pieceCode, origin, key).isEmpty()) {
                    coordinatesToRemove.add(origin);
                }
            });
    ...
    private void findPromotionPaths(BoardInterface board, Map<String, Path> potentialPromotions) {
        ...
        this.promotedPieceMap.forEach((origin, value) ->
                potentialPromotions.forEach((key, value1) -> {
                    int y = origin.getY();
            if ((y == HeuristicsUtil.FIRST_RANK_Y && key.charAt(key.length() - 1) == 'w') ||
                    (y == HeuristicsUtil.FINAL_RANK_Y && key.charAt(key.length() - 1) == 'b')) {
                return;
            }
            String pieceName = key.substring(0, key.length() -
                    (key.charAt(0) == 'b' ? 2 : 1));
            String pieceCodeTemp = HeuristicsUtil.PIECE_CODES.get(pieceName);
    ...
    private void findFromOrigin(BoardInterface board, int originX, boolean white, boolean cage) {
        ...
        Coordinate start = new Coordinate(originX, white ? HeuristicsUtil.FIRST_RANK_Y : HeuristicsUtil.FINAL_RANK_Y);
        String pieceName = HeuristicsUtil.STANDARD_STARTS.get(originX);
