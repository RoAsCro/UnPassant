...
public class PromotionMap extends AbstractDeduction {
    ...
    private void originClaim(Map<Path, List<Path>> pieceSquareOrigin,
        ...
        Path originPool = Path.of(this.origins.stream()
                .filter(c -> c.getY() ==(white ? HeuristicsUtil.WHITE_PAWN_Y : HeuristicsUtil.BLACK_PAWN_Y)).toList());
    ...
    private void reduceClaims(boolean white, Map<Path, List<Path>> pieceSquareOrigin, BoardInterface board) {
        int maxCaptures = ((HeuristicsUtil.MAX_PIECES - this.detector.getCaptureData().getNonPawnCaptures(white)
                .stream().filter(c -> !((c.getX() == HeuristicsUtil.K_BISHOP_X || c.getX() == HeuristicsUtil.Q_BISHOP_X)
                        && !this.detector.getPieceData().getCaged().get(c)))
                .toList().size())
    ...
    private Map<Path, List<Path>> mapPieceSquareOrigin(BoardInterface board, boolean white) {
        ...
        this.goalOrigins.entrySet().stream()
                .filter(entry -> entry.getKey().getY() == (white ? HeuristicsUtil.FINAL_RANK_Y : HeuristicsUtil.FIRST_RANK_Y)) // Correct colour
                .filter(entry -> {
    ...
    private void setGoalOriginsInit(BoardInterface board) {
        ...
        List<Coordinate> whiteOrigins = this.origins.stream()
                .filter(coordinate -> coordinate.getY() == HeuristicsUtil.WHITE_PAWN_Y).toList();
        List<Coordinate> blackOrigins = this.origins.stream()
                .filter(coordinate -> coordinate.getY() == HeuristicsUtil.BLACK_PAWN_Y).toList();
        List<Coordinate> whiteTargets = this.targets.stream()
                .filter(coordinate -> coordinate.getY() == HeuristicsUtil.FINAL_RANK_Y).toList();
        List<Coordinate> blackTargets = this.targets.stream()
                .filter(coordinate -> coordinate.getY() == HeuristicsUtil.FIRST_RANK_Y).toList();
    ...
    private void setGoalOrigins(Coordinate origin,
        ...
        Path shortest = new PathfinderUtil(this.detector).findShortestPawnPath(
                ...
                origin.getY() == HeuristicsUtil.BLACK_PAWN_Y || origin.getY() == HeuristicsUtil.FIRST_RANK_Y, false,
                origin.getY() == HeuristicsUtil.WHITE_PAWN_Y || origin.getY() == HeuristicsUtil.FINAL_RANK_Y
                        ? forbiddenBlackPaths
                        : forbiddenWhitePaths);

        if (!shortest.isEmpty()) {
            boolean white = origin.getY() == HeuristicsUtil.FINAL_RANK_Y;
            Path cagedCaptures = Path.of(this.detector.getCaptureData().getNonPawnCaptures(white)
                    .stream().filter(c -> c.getX() == HeuristicsUtil.K_ROOK_X || c.getX() == HeuristicsUtil.Q_ROOK_X).toList());
            if (cagedCaptures.size() > 0 && (white || origin.getY() == 0)) {
                int captureY = white ? HeuristicsUtil.BLACK_ESCAPE_Y : HeuristicsUtil.WHITE_ESCAPE_Y;
    ...
    private class PromotionCombinedPawnMap extends CombinedPawnMap {
        ...
        /**
         * Overrides deduce(), ensuring that bishops that aren't captured by pieces on the board are still captured
         * @param board the board to be checked
         */
        @Override
        public void deduce(BoardInterface board) {
            super.deduce(board);
            List<Coordinate> list = PromotionMap.this.detector.getCaptureData().getNonPawnCaptures(true)
                    .stream().filter(c -> ((c.getX() == HeuristicsUtil.K_BISHOP_X || c.getX() == HeuristicsUtil.Q_BISHOP_X)
                            && !PromotionMap.this.detector.getPieceData().getCaged().get(c)))
                    .toList();
            ...
            List<Coordinate> capturedTwo = new LinkedList<>();
            List<Coordinate> listTwo = PromotionMap.this.detector.getCaptureData().getNonPawnCaptures(false)
                    .stream().filter(c -> ((c.getX() == HeuristicsUtil.K_BISHOP_X || c.getX() == HeuristicsUtil.Q_BISHOP_X)
                            && !PromotionMap.this.detector.getPieceData().getCaged().get(c)))
                    .toList();
            ...
            if ((!list.isEmpty() || !listTwo.isEmpty())
                    && (!list.stream().filter(c -> !captured.contains(c)).toList().isEmpty()
                    || !listTwo.stream().filter(c -> !capturedTwo.contains(c)).toList().isEmpty())) {
                this.state = true;
                super.deduce(board);
            }
        }
            ...
            for (int i = 0; i < path.size() - 1; i++) {
                if (path.get(i).getX() != path.get(i+1).getX()) {
                    for (Coordinate c : list) {
                        if (Coordinates.light(path.get(i)) == Coordinates.light(c)) {
    ...
    private class PromotionPawnMap extends CombinedPawnMap.PawnMap {
        ...
        private void flip(boolean direction) {
            ...
            getPawnOrigins().entrySet().stream().filter(entry -> {
                        ...
                        return (direction ? value.getFirst().getY() : entry.getKey().getY())
                                == (getColour() ? HeuristicsUtil.FINAL_RANK_Y : HeuristicsUtil.FIRST_RANK_Y);})
                    .forEach(entry -> {
                        Path value = entry.getValue();
        ...
        protected void rawMap(BoardInterface board) {
            ...
            int y = (getColour() ? HeuristicsUtil.FINAL_RANK_Y : HeuristicsUtil.FIRST_RANK_Y);

            Path targetsFiltered = Path.of(origins.stream().filter(coordinate ->
                    coordinate.getY() == Math.abs(y - (HeuristicsUtil.BLACK_PAWN_Y))).toList());

            targets.stream().filter(coordinate -> coordinate.getY() == y)
                    .forEach(c -> toAddTwo.put(c, Path.of(targetsFiltered.stream()
                            .filter(cTwo -> Math.abs(c.getX() - cTwo.getX()) < HeuristicsUtil.K_ROOK_X).toList())));
        ...
        protected void captures(Map<Coordinate, Path> origins) {
            ...
            getPawnOrigins().entrySet().stream()
                    .filter(entry -> entry.getKey().getY() == (getColour() ? HeuristicsUtil.FINAL_RANK_Y : HeuristicsUtil.FIRST_RANK_Y))
                    .forEach(entry -> {
